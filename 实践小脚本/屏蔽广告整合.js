...
// @version           4.778
// @description      新增twitter和Youtube微博视频下载以及广告过滤，可按网址和关键词屏蔽过滤百度谷歌必应bing360微信微博知乎豆瓣B站的搜索结果、屏蔽谷歌推广的广告和去知乎广告、csdn广告、去百度搜索广告、去百度知道广告、去百度文库广告、去360搜索广告、去必应Bing搜索广告、百度文库去广告、去百度在别的网站的推广广告，移除知乎的强制登陆弹窗。去豆瓣广告、去B站广告
...
2022.12.21 隐藏kemono.party广告


说明：
百度等网站有可能随时更新可能导致部分功能有些兼容问题需要更多功能和建议的话，欢迎跟帖反馈，
...
(function () {
  ...
  var configm = 'aHR0cHM6Ly93d3cuZGFkaXlvdWh1aTAxLmNuL2FwcC90b29sL3YzLWJkdG9vbC5odG1s';
  configm = window.atob(configm);
  GM_registerMenuCommand("【♐  网页广告过滤设置】", function () {
  ...
  if (GM_getValue('allconfig').baidu.baiducss == true) {
    ...
    if (zkdurldomain.indexOf('bing.com/search?q=') > 0 && !isMobile()) {
  ...
  function delother() {
    ...
    if ($("input[type='password']").length > 0 || $("input[name='password']").length > 0 || $("input[type='email']").length > 0 || $("input[name='email']").length > 0 || $("input[name='code']").length > 0 || $("input[name='phone']").length > 0 || $("input[placeholder*='手机']").length > 0 || $("input[placeholder*='邮箱']").length > 0 || $("input[placeholder*='账号']").length > 0 || $("input[placeholder*='密码']").length > 0 || $("input[placeholder*='验证']").length > 0) {
      kgpingbiswhitezt = false;
    }
    //说明：此处是为了匹配某些网站登陆时插件误杀登陆验证的页面需要，防止不能正常登陆。并无没有收集用户的任何信息
    if (GM_getValue('allconfig').baidu.kgpingbis == true) {
      ...
      $("iframe").each(function () {
        if ($(this).attr("src")) {
          ...
          if ($(this).attr("src").indexOf("http") == 0 && $(this).attr("src").indexOf(iframehost) < 0 && kgpingbiswhitezt) {
            $(this).remove();
          }
        } else {
          $(this).remove();
    ...
    if (zkdurldomain.indexOf(csdn) >= 0) {
      ...
      $('body').on('click', '.hljs-button.signin,.unlogin-comment-bt', function () {
